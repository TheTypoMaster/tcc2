...
\begin{table}[htb]
\IBGEtab{%
	\caption{Parâmetros utilizados no MTD1}%
	...
	\midrule
	$Th_k$ & Valor de \emph{threshold} para a iteração $k$ \\
	\midrule 
	$q$ & Fração de $Th_{k-1}$ para determinação de $Th_k$ \\
	\midrule 
	$Nc_{k}$ & Número total de candidatos para centros de segmentos identificados na iteração $k$ \\
	\midrule 
	$r_k$ & Razão entre número de candidatos identificados na iteração $k$ e o comprimento total do sinal \\
	\midrule 
	$r_target$ & Razão mínima esperada para r_k, utilizada para determinar o final do método \\
	\bottomrule
\end{tabular}%
}{%
...
\begin{figure}
\centering
\includegraphics[width=0.6\linewidth]{../img/mtd1example.eps}
